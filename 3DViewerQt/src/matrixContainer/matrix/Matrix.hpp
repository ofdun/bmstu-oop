    ...
    DeterminantComputable<T> && std::is_floating_point_v<T>
{
    ...
    for (auto k = 0; k < n - 1; k++)
    {
        if (std::abs(tmpMatrix[k][k]) < __DBL_EPSILON__)
        {
            bool found = false;
            for (auto i = k + 1; i < n && !found; i++)
            {
                if (std::abs(tmpMatrix[i][k]) < __DBL_EPSILON__)
                {
                    tmpMatrix.swapRows(k, i);
                    found = true;
                }
...
{
    return std::ranges::all_of(*this, [](const auto &e) {
        return std::abs(e) < __DBL_EPSILON__;
    });
}
    ...
    IdentityMatrixCompatible<T> && std::is_floating_point_v<T>
{
    ...
    return std::ranges::all_of(*this, [this, &cur](const auto &elem) {
        ...
        cur++;
        if (r == c)
            return std::abs(elem - value_type{1}) < __DBL_EPSILON__;
        return std::abs(elem) < __DBL_EPSILON__;
    });
}
...
template <Storable T> std::pair<Matrix<double>, Matrix<double>> Matrix<T>::LU() const requires
    LUComputable<T> && std::is_arithmetic_v<T>
{
    if (!square())
        throw IncompatibleSizeException(__FILE__, __FUNCTION__, __LINE__, time(nullptr));
...
Matrix<T>::Matrix(const range &rng, size_t rows, size_t columns) requires std::convertible_to<
    std::ranges::range_value_t<range>, T>
{
    if (rows <= 0 || columns <= 0)
        throw InvalidInitSizeException(__FILE__, __FUNCTION__, __LINE__, time(nullptr));
